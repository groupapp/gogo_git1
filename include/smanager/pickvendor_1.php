...
switch ($mode) {
	...
	case "search":
		$strSQL = "SELECT DISTINCT BrandName FROM Products Where BrandName LIKE '".$keyword."%' AND BrandName IS NOT NULL AND BrandName<>'' ORDER BY BrandName ASC";
		//$strSQL = "SELECT * FROM Pack WHERE PackChartName LIKE '".$keyword."%' ORDER BY PackChartID ASC";	
		$DB->query($strSQL);
		
		$json = '{"vendor": [';
		while ($data = $DB->fetch_array($DB->res)) {
			if ($cnt>0) $json .=",";
			$json .= '{"scode":"'.$data["BrandName"].'", "sname":"'.$data["BrandName"].'"}';
			$cnt++;
		}
		$json .= ']}';
	break;
