	...
	$step			= empty($_REQUEST["stepby"])		? null : $_REQUEST["stepby"];
	...
	//=====================item add==================================
	$itemadd			= empty($_REQUEST["itemadd"])		? null : $_REQUEST["itemadd"];
	$category			= empty($_REQUEST["category"])		? null : $_REQUEST["category"];
	$name_e			= empty($_REQUEST["name_e"])		? null : $_REQUEST["name_e"];
	$name_k			= empty($_REQUEST["name_k"])		? null : $_REQUEST["name_k"];
	$name_s			= empty($_REQUEST["name_s"])		? null : $_REQUEST["name_s"];
	$name_c			= empty($_REQUEST["name_c"])		? null : $_REQUEST["name_c"];
	...
	if($action=="step1")
	{
		if($mcontact_id>0)
		{
			$strSQLy		= "UPDATE contact_master SET
						from_date				= \"".$from_date."\",
						to_date				= \"".$to_date."\",
						from_time				= \"".$from_time."\",
						to_time				= \"".$to_time."\",
		...
		else if($from_date!="")
		{
			$strSQLx		= "INSERT INTO contact_master (id_vendor,id_location,from_date,to_date,from_time,to_time,sfrom_date,sto_date,date_created,date_updated,id_user_created,id_user_updated)  VALUES(
						...
						$strSQLa = " Select contact_id from contact_master Order BY contact_id DESC Limit 0,1";
						$DBa->query($strSQLa);
		...
		if (!empty($_FILES['csv']['size']) > 0) { 
				...
				do { 
					if ($data[0]) { 
						mysql_query("INSERT INTO contacts (category, name, unit,qty,price,description,mcontact_id) VALUES 
							( 							
								'".addslashes($data[0])."', 
								'".addslashes($data[1])."', 
								'".addslashes($data[2])."', 
	...
	else if(($action =="view") or ($action =="step2"))
	{
		...
		if($itemadd==1)
		{
			$strSQdi = "INSERT INTO contacts (category, name, unit,qty,price,description,mcontact_id) VALUES 
			( 							
								'".$category."', 
								'".$name_e."', 
								'".$unit."', 
		...
		else
		{
			...
			$strSQLm = "SELECT * FROM contact_master WHERE contact_id=".$mcontact_id;
	...
	if($delchk==1)
	{
		$strSQLd = "DELETE FROM contacts WHERE contact_id=".$icontact_id;
	...
	if($action == "del") {
		...
		foreach($_POST["delid"] as $key => $val) {
			$strSQL = ("DELETE FROM contact_master WHERE contact_id=".$val);
		//echo $strSQL;
			$DB->query($strSQL);

			$strSQL1 = ("DELETE FROM contacts WHERE mcontact_id=".$val);
		//echo $strSQL;
			$DB1->query($strSQL1);
		}
	}
	...
	foreach($_FILES as $name => $value) {
		...
		if(!empty($_POST[$name.'_values']))
		{
			...
			*/
			...
			$strSQLi = "UPDATE contacts SET img= \"".$fname."\"	, rimg= \"".$tmp[1]."\"  WHERE contact_id=".$json->alt ;					
	...
	<div id="addarea" style="display:none;">			
				...
				<div class="portlet box grey-cascade">						
						...
						<div class="portlet-body form" id="addnew" style="display:block;">
							<form enctype="multipart/form-data" action="saleitem.php" method="get"  role="form" class="form-horizontal form-row-seperated">
								<input type="hidden" name="action" value="step1">
								<input type="hidden" name="menu" value="1">
								<input type="hidden" name="stepby" value="1">
								<div class="form-body">
			...
			<div class="portlet box red-sunglo" >						
						...
						<div class="portlet-title">
							...
							<div class="form-group" style="display:block">
									...
									<div class="btn-group">
											<a href="edit_item.php?mcontact_id=<?php echo $mcontact_id?>" class="ajax"><span class="btn green">Add New Item</span> </a>
										</div>
									<div class="btn-group">
									<?php $xcategory			= empty($_REQUEST["xcategory"])		? null : $_REQUEST["xcategory"]; ?>
												...
												<div class="col-md-7">
													<select id="xcategory" onchange="change_catagory(<?php echo $mcontact_id?>)" class="form-control select2me" data-placeholder="Select...">
								...
								$strSQL = "SELECT * FROM contacts WHERE 1 ";
							...
							//print_r($strSQL);		
								...
								echo "<table>
								<tr><td>";
								echo '<form enctype="multipart/form-data" action="saleitem.php" method="get" role="form" id="form2" name="forms2">';
								...
								echo '<input type="hidden" name="stepby" value="2">';
								...
								while ($data = $DB->fetch_array($DB->res)){			
									...
									*/
									...
									echo '<td><a class="ajax" href="edit_item.php?contact_id='.$data['contact_id'].'&mcontact_id='.$data['mcontact_id'].'">';
									...
									echo '<td class="col-xs-1">
											<a class="ajax btn green" href="edit_item.php?contact_id='.$data['contact_id'].'&mcontact_id='.$data['mcontact_id'].'&menu=1" >Edit</a>
										</td>';
									echo '<td class="col-xs-1">
											<a  href="saleitem.php?icontact_id='.$data['contact_id'].'&mcontact_id='.$data['mcontact_id'].'&del=1&menu=1&view=1&uploadchk=1" class="btn green">Del</a>
										</td>';
									echo '</tr></table></li>';
									$i++;
...
<div id="brancharea" > 						
							...
							<div class="portlet box purple" >
								...
								<form enctype="multipart/form-data" action="saleitem.php" method="post" role="form">	
									<div class="col-md-10">
									<div class="btn-group">
										<button id="sample_editable_1_new" class="btn green" onclick="return delitem(this.form)" style="margin-left: 10px;">
									...
									<tbody>	
									<?php
									$strSQL = "SELECT * FROM contact_master";
									$DB->query($strSQL);
									while ($data = $DB->fetch_array($DB->res)){
										...
										echo '<tr class="odd gradeX">
											...
											<td>
												<a href="saleitem.php?mcontact_id='.$data['contact_id'].'&action=view&menu=1&view=1&uploadchk=1" >
...
<script src="../../assets/js/html5imageupload.js?v1.4.3"></script>
	...
	function change_catagory(mcontact_id){
	//alert('xxxx');
	var xcategory=$('#xcategory').val();
	location ='saleitem.php?mcontact_id='+mcontact_id+'&menu=1&xcategory='+xcategory+'&action=view&menu=1&view=1&uploadchk=1';
	}
