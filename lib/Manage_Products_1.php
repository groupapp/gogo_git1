...
<div id="bodywrapper">
	...
	<!-- sideMenu -->
	<div id="navLeft">
		<div style="font: bold 16px/1.2 Palatino Linotype; color: #aaa;">Lemontreeclothing
			Events</div>
	...
	<div id="contwrapper">
		...
		<div id="sub2">
			<form name="Load" method="post" action="Manage_Products.php">
				<p><b class="bold_s">Copy From Existing Item</b> &nbsp; Product ID : 
			...
			<form name="ProductDetail_Form" method="post" enctype="multipart/form-data" action="Manage_Products_action.php">
				...
				<p style="margin-left: 11px; padding: 5px 0;">Fields with <span class="redstar">*</span> below are required fields, meaning that you must select one or write something.</p>
					...
					/*			if($Edit_Player1==null){
						...
						<tr class="thin_border_bottom">
							...
							<td colspan="3" class="general">
								...
								<select name="Edit_SizeChartID" style="width: 100%; font: normal 11px tahoma, sans-serif;">
									...
									<?php 
										$strSQL = "SELECT * FROM SizeCharts ORDER BY SizeChartName ASC";
										$DB->query($strSQL);
										$sortNum = 1;
										$sizeList;
										while ($data = $DB->fetch_array($DB->res)){
										...
										//	echo "<option>" . $sizeList . "</option>";
											...
											echo ">(" . $data["SizeChartID"] . ")&nbsp;" . $data["SizeChartName"] . ":&nbsp;" . $sizeList . "</option>";
											$sortNum++;
											$sizeList = "";
										}
									?>
						...
						<tr class="thin_border_bottom">
							...
							<td colspan="3" class="general">
								...
								<span id="ColorsAvailable" style="display: block;">
...
?>
						...
						var colorid=frm.AddColorID.value;
						
						location='Manage_colorproducts_action.php?act=view&aid='+ProductID+'&Display_ActiveItems_By=&Display_ActiveItems_In=&pp=&ppAI=1&ppII=&action=add&colorid='+colorid;
						
						}
						function d(frm){
						if (confirm("Do you want to remove?")) {
							var delProductID=frm.ProductID.value;
							var delcolorid=frm.DelColorID.value;
							location='Manage_colorproducts_action.php?act=view&aid='+delProductID+'&Display_ActiveItems_By=&Display_ActiveItems_In=&pp=&ppAI=1&ppII=&action=del&delcolorid='+delcolorid;
						}
						return false;
						...
						$('#AddColorID').click(function() {
							...
							var productid = $('#proID').val();
								...
								$.ajax({
											...
											data:{"mode":"add","colorid":colorid,"productid":productid},
											success:function(d) {
												if (d) {
						...
						$('#DelColorID').click(function() {
							...
							var productid = $('#proID').val();
								$.ajax({
											async:false, type:"post", dataType:"json", url:"pickcolor.php",
											data:{"mode":"del","delcolorid":delcolorid,"productid":productid},
											success:function(d) {
												if (d) {
						...
						<tr class="thin_border_bottom">
							<th class="subject2">Personalize</th>
							<td colspan="3" class="general">
						...
						<tr class="thin_border_bottom">
							...
							<td colspan="3" class="general">
								...
								<select name="Edit_QuantityDiscountID">
									...
									<?php 
										$strSQL = "SELECT * FROM QuantityDiscounts ORDER BY QD_ID ASC";
										$DB->query($strSQL);
										$discountList;
										while ($data = $DB->fetch_array($DB->res)){
											for($i=1; $i<=6; $i++){
												$discountList .= " " . $data["LowerQty".$i] . " ~ " . $data["UpperQty".$i] . " (" . $data["DiscountPercentage".$i] . " %)";
											}								
											echo "<option value=\"" . $data["QD_ID"] . "\"";
											if($dataProd["QuantityDiscountID"]==$data["QD_ID"]){
												echo "selected";
											}
											echo ">(" . $data["QD_ID"] . ")&nbsp;" . $data["QuantityDiscountName"] . ":&nbsp;" . $discountList . "</option>";
											$sortNum++;
											$discountList = "";
										}
									?>
						...
						<?php 
							for($i=1; $i<=15; $i++){
								...
								echo "</th>";
								echo "<td colspan=\"3\" class=\"general\">&nbsp; ";
