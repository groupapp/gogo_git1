...
namespace example_mvc4
{
    public class TFile
    {
        ...
        public async Task<bool> UploadAsChunk(Stream InputStream, string path, int position)
        {
            bool result = false;
            path = this._Path + path;
            if(File.Exists(path))
            {
                await TFile.StreamToFileResumable(InputStream, path, position);
            }
            else
            {
                await TFile.StreamToFileAsync(InputStream, path);
            }
        ...
        public async static Task<bool> StreamToFileAsync(Stream stream, string path)
        {
            bool result = false;
        ...
        public async static Task<bool> StreamToFileResumable(Stream stream, string path, int position)
        {
            bool result = false;
