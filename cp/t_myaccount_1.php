...
//print_r($strSQL);	
 ...
 ?> 
     ...
     <tr onmouseover="this.style.backgroundColor='#ccffff'" onmouseout="this.style.backgroundColor=''">
      <td align="center"><input type="hidden" id="flag_<?php echo $data_s["cUser_Num"]?>" value="0"><?php echo $n?></td>
      <td align="left"><img id="expand_<?php echo $data_s["cUser_Num"]?>" src="/img/incusersingle.png" onclick="showDown('<?php echo $data_s["cUser_Num"]?>')"><span id="coll_<?php echo $data_s["cUser_Num"]?>" style="display:none" onclick="HideDown('<?php echo $data_s["cUser_Num"]?>')">-</span><?php echo $data_s["cUser_Num"] ?></td>
...
var flag_chk=$('#flag_'+cUser_Num).val();	
	
	/*if(flag_chk==1)
	{
		$('#down_'+cUser_Num).css('display','block');
		//$('#flag_'+cUser_Num).val(0);
	}
	else
	{*/	
		$.ajax({
