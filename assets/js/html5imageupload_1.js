...
//		this.button.rotateccw	= '<div class="btn btn-default btn-rotate-ccw" title="' + (this.options.ccwTitle || 'Rotate counter clockwise') + '"><i class="glyphicon glyphicon-share icon-flipped"></i></div>';
	...
	$.html5imageupload.prototype = {
		...
		readImage: function(image, src, name, mimeType) {
			...
			/*name.replace(/modules/g,'xx');

			alert(name);


			var str =name;
			var res = str.replace(/tmp/gi, "");
			alert(res);*/
    		...
    		img.onload 		= function(tmp) {
    			...
    			if (options.originalsize == false) {
    				...
    				useWidth		= $(element).outerWidth() + 0;//40
    				useHeight		= useWidth / ratio;
    				
    				if (useHeight < $(element).outerHeight()) {
    					useHeight	= $(element).outerHeight() + 0;//40
    			...
    			var left			= parseFloat(($(element).outerWidth() - useWidth) / 2) //* 2;
    			var top				= parseFloat(($(element).outerHeight() - useHeight) / 2) //* 2;
...
//alert(factor);
			...
			var obj					= {name: $(image).attr('name'),alt: $(input).attr('alt'), imageOriginalWidth: imageOriginalWidth, imageOriginalHeight: imageOriginalHeight, imageWidth: imageWidth, imageHeight: imageHeight, width: finalWidth, height: finalHeight, left: finalLeft, top: finalTop}
...
//alert('b');
	        				...
	        				$(input).after($('<input type="text" name="' + $(input).attr('name') + '_values" alt="' + $(input).attr('alt') + '"  class="final" />').val(json));
...
//alert(options.removeurl+'/xx'+$(_self.image).attr('name'));	


var str =$(_self.image).attr('name');

			var res = str.replace(/kmanager/gi, '');
			var res1 = res.replace(/tmp/gi, '');
			...
			if (options.removeurl != null && !empty(str)) {//&& !empty($(_self.image).attr('name'))
			//alert('abab');	
